...
public interface WithdrawalRequestRepository extends JpaRepository<WithdrawalRequestJpaEntity, UUID> {
    Page<WithdrawalRequestJpaEntity> findByShopIdOrderByCreatedAtDesc(UUID shopId, Pageable pageable);
    Page<WithdrawalRequestJpaEntity> findByStatusOrderByCreatedAtDesc(String status, Pageable pageable);
}
